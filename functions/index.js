...
const fetchNaverSearch = async () => {
  ...
  const queries = Object.values(CAREER_KEYWORDS)
    .flatMap((keywords) => keywords.slice(0, 2).map((keyword) => `${keyword} 공모전 해커톤 대외활동`));
  const uniqueQueries = uniq(queries).slice(0, 12);
  const results = [];
  ...
  for (const query of uniqueQueries) {
    ...
    if (!response.ok) {
      logger.warn('Naver Search API 응답 오류', { status: response.status, query });
      continue;
    }
    ...
    results.push(...toArray(data.items).map((item) => ({
      ...
      source: getDomain(item.originallink || item.link) || 'Naver Search',
      sourceType: 'naver-search-api',
    })));
  }
